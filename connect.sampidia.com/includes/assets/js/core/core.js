...
// initialize API URLs
var api = [];
window.CORE_JS_VERSION = '1.0.3';
console.log("core.js loaded version:", window.CORE_JS_VERSION);
/* core */
...
api['payments/nowpayments'] = ajax_path + "payments/nowpayments.php";
...
$(function () {
  ...
  /* CoinPayments */
  /* NOWPayments */
  $('body').on('click', '.js_payment-nowpayments', function () {
    console.log("NOWPayments button clicked");
    var _this = $(this);
    var data = {};
    data['handle'] = _this.data('handle');
    if (data['handle'] == "packages") {
    ...
    button_status(_this, "loading");
    /* post the request */
    $.post(api['payments/nowpayments'], data, function (response) {
      console.log("NOWPayments AJAX Response:", response);
      /* button reset */
      button_status(_this, "reset");
      /* check the response */
      if (response.nowpayments_url) {
        window.location.href = response.nowpayments_url;
      }
      /* check if there is a callback */
      if (response.callback) {
        eval(response.callback);
      ...
      .fail(function (xhr, status, error) {
        console.error("NOWPayments AJAX Fail:", status, error, xhr.responseText);
        /* button reset */
        button_status(_this, "reset");
        /* handle error */
        modal('#modal-message', { title: __['Error'], message: __['There is something that went wrong!'] });
