  ...
  switch ($_GET['do']) {
    case 'add_movie':
      ...
      if (isset($_POST['source_url']) && !is_empty($_POST['source_url'])) {
        ...
        $source_type = get_video_type($url);
        if ($source_type == 'youtube') {
          /* extract and normalize YouTube URL to prevent redirect corruption */
          $youtube_id = get_youtube_id($url, false);
          if (empty($youtube_id)) {
            throw new Exception(__("Could not extract YouTube video ID from the provided URL"));
          }
          $source = 'https://www.youtube.com/watch?v=' . $youtube_id;
        } elseif ($source_type == 'vimeo') {
          /* keep Vimeo URL as-is, no redirect resolution needed */
          $source = $url;
        } else {
