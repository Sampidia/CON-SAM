...
try {
  ...
  switch ($_GET['view']) {
    ...
    case 'ads':
      ...
      switch ($_GET['sub_view']) {
        ...
        case 'edit':
          ...
          $data = $get_data->fetch_assoc();
          $data['code'] = html_entity_decode($data['code'], ENT_QUOTES);
    ...
    case 'nowpayments':
      ...
      // page header
      page_header($control_panel['title'] . " &rsaquo; " . __("NOWPayments Transactions"));

      // get data
      $nowpayments_transactions = $user->get_nowpayments_transactions(true);

      // assign variables
      $smarty->assign('nowpayments_transactions', $nowpayments_transactions);
      break;
  ...
  if ($user->_is_admin) {
    ...
    /* nowpayments insights */
    $get_nowpayments = $db->query("SELECT COUNT(*) as count FROM nowpayments_transactions WHERE status = '0' OR status = '1'");
    $nowpayments_insights = $get_nowpayments->fetch_assoc()['count'];
    $smarty->assign('nowpayments_insights', $nowpayments_insights);
  }
  /* reports insights */
