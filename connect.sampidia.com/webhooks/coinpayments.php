...
try {
  ...
  // get post data
  if ($system['coinpayments_api'] == "v2") {
    $input_data = file_get_contents('php://input');
    $post_data = json_decode($input_data, true);
    $custom_id = $post_data['customId'] ?? $post_data['items'][0]['customId'] ?? null;
    $txn_id = $post_data['invoiceId'] ?? $post_data['id'] ?? null;
  } else {
    $post_data = $_POST;
    $custom_id = $post_data['custom'];
    $txn_id = $post_data['txn_id'];
  }

  // handle the transaction
  $transaction = $user->get_coinpayments_transaction($custom_id, $txn_id);
  if ($transaction) {
    switch ($_GET['handle']) {
      case 'packages':
        // valid inputs
